...
export default function DirectorGeneratePage() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading("Processing assessment questions...");
    try {
      ...
      if (res.ok && data.success) {
        ...
        markedQuestions.forEach((q, idx) => {
          ...
          fetch("/api/director/generate-diagram-image", {
            method: "POST",
  ...
  const openSaveModal = () => {
    ...
    if (firstUnconfiguredIdx !== -1) {
      ...
      const errorMsg = `Question #${qNum} has no correct answer configured. Please configure the correct answer before saving.`;
      setValidationError(errorMsg);
      toast.error(errorMsg);

      setTimeout(() => {
        const el = document.getElementById(`director-question-card-${firstUnconfiguredIdx}`);
        if (el) {
  ...
  const handleSaveAssessment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cleanQuestions = finalQuestions.map(
        ({ imageGenerating, imagePending, ...rest }) => {
    ...
    <div className="p-8 container mx-auto space-y-8 animate-in fade-in duration-300">
        ...
        <div className="lg:col-span-2 space-y-6 no-print">
          <form ref={formRef} onSubmit={handleGenerate} className="space-y-6">
                    ...
                    {[
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <input
                            type="radio"
                            name="directorPdfClassification"
                            value={opt.id}
            ...
            pdfClassification !== "syllabus" ? (
              <div className="bg-blue-50/70 border border-blue-200 p-5 space-y-3 shadow-sm rounded-none">
                <div className="flex items-center gap-2">
                  <FileText size={16} className="text-blue-600" />
              ...
              <div className="bg-white/80 border border-gray-200 rounded-none p-6 space-y-5 shadow-sm backdrop-blur-sm">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-200 px-2.5 py-1">
                      ...
                      <input
                        ...
                        className="w-12 text-center text-xs font-bold text-gray-900 bg-white border border-blue-300 px-1 py-0.5 outline-none focus:border-blue-600"
                      />
          ...
          {generatorLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 no-print">
                {validationError && (
                  <div className="bg-red-50 border-2 border-red-500 p-4 text-xs text-red-800 font-bold flex items-center justify-between gap-3 shadow-sm animate-in fade-in slide-in-from-top-2 duration-150">
                    <div className="flex items-center gap-2">
                ...
                {generatedQuestions.map((q, idx) => (
                  <div
                    id={`director-question-card-${idx}`}
                    key={q.id}
                    className={`rounded-none p-5 space-y-4 relative shadow-sm backdrop-blur-sm transition-all ${
                  ...
                  >
                    {highlightedQuestionIdx === idx && (
                      <div className="bg-red-600 text-white px-3 py-1.5 text-xs font-bold flex items-center justify-between gap-2 shadow-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {q.requiresReview && (
                        <span className="text-[9px] font-black uppercase bg-amber-100 text-amber-800 border border-amber-300 px-1.5 py-0.5 rounded-none">
                          Needs Review
                        </span>
                      )}
                    ...
                    {(isDiagramType(q.type) ||
                      ...
                      <div className="space-y-2 print:hidden">
                        {q.imageGenerating ? (
                          <div className="h-48 bg-gradient-to-r from-violet-50 via-purple-50 to-violet-50 border border-violet-200 rounded-none flex flex-col items-center justify-center gap-3 p-4 text-center animate-pulse">
                            <div className="flex items-center gap-2">
                              <RefreshCw
                                size={18}
                            ...
                            <p className="text-[11px] text-gray-500 max-w-xs">
                              Synthesizing visual SVG diagram for this question.
                            </p>
                            <button
                              type="button"
                              onClick={() => handleCancelImageGeneration(idx)}
                              className="mt-1 bg-white border border-red-300 text-red-600 hover:bg-red-50 text-[11px] font-bold px-3 py-1.5 rounded-none shadow-sm transition-colors flex items-center gap-1.5 cursor-pointer"
                            >
                              <X size={13} />
                              <span>Cancel & Upload Manually</span>
                            </button>
                          </div>
                          ...
                          <div className="border border-dashed border-violet-300 p-6 flex flex-col gap-3 bg-violet-50/10">
                            ...
                            <div className="space-y-2">
                              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-500 block">
                                Upload image file, paste web link, or generate
                                via AI
                              </label>
                              <div className="flex flex-wrap sm:flex-nowrap gap-2">
                                <input
                                  ...
                                  className="flex-1 min-w-[200px] bg-white border border-gray-300 rounded-none px-3 py-1.5 text-xs text-gray-900 font-medium outline-none focus:border-blue-600"
                                />
                                <label
                                  className="bg-white border border-gray-300 text-gray-700 hover:text-blue-600 hover:border-blue-300 px-3 py-1.5 rounded-none shadow-sm cursor-pointer flex items-center gap-1 text-xs font-bold transition-colors"
                                ...
                                <button
                                  ...
                                  className="bg-violet-600 hover:bg-violet-700 text-white px-3 py-1.5 rounded-none shadow-sm cursor-pointer flex items-center gap-1.5 text-xs font-bold transition-colors"
                                  title="Synthesize diagram using AI"
                                >
                                  <RefreshCw size={13} />
                            ...
                            {q.options.map((opt, oIdx) => {
                              ...
                              return (
                                ...
                                >
                                  <input
                                    type="radio"
                                    name={`dir_q_${idx}_correctAnswer`}
                                    value={opt || letter}
                                    checked={isSelected}
                                    onChange={() => {
                                      const updated = [...generatedQuestions];
                                  ...
                                  <span className="text-xs truncate">
                                    ...
                                    {opt ? (opt.length > 20 ? opt.slice(0, 20) + "…" : opt) : `Option ${letter}`}
                                  </span>
                                </label>
                              );
                            })}
      ...
      {showReassignModal && selectedImageForReassign && (
        <div className="fixed inset-0 z-50 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4 no-print animate-in fade-in duration-200">
          <div className="bg-white border border-gray-200 w-full max-w-md rounded-none p-6 space-y-4 shadow-2xl">
