...
export default function DirectorLiveSessionDetailPage({
  params,
}: {
    ...
    try {
      ...
      const data = session.participants.map((p: any, idx: number) => {
        const scorePercentage =
          totalQuestionsCount > 0 && p.score !== null
            ? Math.round((p.score / totalQuestionsCount) * 100)
            : 0;
        return {
          ...
          "Integrity Status": p.terminated
            ? "Disqualified"
            : p.tabSwitches > 0
              ? `${p.tabSwitches} Tab Switch Warning(s)`
              : "Verified Secured",
      ...
      XLSX.writeFile(
        workbook,
        `${session.assessment.title.replace(/\s+/g, "_")}_Results.xlsx`,
      );
      ...
      toast.update(toastId, {
        type: "error",
        message: `Failed to export Excel spreadsheet: ${err.message || err}`,
      });
    }
  };
  ...
  const handleShareReport = async (participant: any) => {
    ...
    const toastId = toast.loading(
      `Sharing assessment report with ${participant.name}...`,
    );
    try {
      ...
      if (res.ok && data.success) {
        toast.update(toastId, {
          type: "success",
          message: `Assessment report successfully emailed to ${participant.name} (${participant.email})!`,
        });
      } else {
        toast.update(toastId, {
          type: "error",
      ...
      toast.update(toastId, {
        type: "error",
        message: "Network error trying to share report.",
      });
    } finally {
  ...
  if (error || !session) {
    return (
      <div className="min-h-screen  flex items-center  justify-center">
        <div className="m-auto p-8 py-16 space-y-6 text-center bg-white max-w-2xl border border-red-200">
          ...
          <p className="text-gray-550 text-sm">
            {error || "The session report is unavailable or access was denied."}
          </p>
          <Link
  ...
  return (
    <div className="p-8 container mx-auto animate-in fade-in slide-in-from-bottom duration-300">
      <Link href="/director/sessions" className="flex items-center gap-2 mb-4">
        {/* Back navigation & Header */}
        <span
          className="flex items-center w-max p-2 bg-white border border-gray-400 rounded-none hover:bg-gray-50 text-gray-500 hover:text-gray-900 transition-colors shadow-sm"
          style={{ borderRadius: "100%" }}
        >
          <ArrowLeft size={16} />
        </span>
        <span className="text-[11px] text-gray-600 ">BACK TO ALL SESSIONS</span>
      </Link>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              ...
              <span className="font-bold text-gray-700">
                {session.assessment.position}
              </span>{" "}
              · Institution:{" "}
              <span className="font-bold text-gray-700">
                {session.assessment.recruitmentFor}
              </span>{" "}
              · Room key:{" "}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white/80 border border-gray-200 rounded-none p-5 shadow-sm backdrop-blur-sm">
          ...
          <p className="text-[10px] text-gray-400 mt-1">
            Accuracy:{" "}
            {totalQuestions > 0
              ? ((session.stats.avgScore / totalQuestions) * 100).toFixed(0)
              : 0}
            %
          </p>
        </div>
      ...
      <div >
        <div className=" mb-6">
          <h3 className="text-lg font-black text-gray-900">
            Teacher Leaderboard &amp; Response Audit
          </h3>
          <p className="text-xs text-gray-500 mt-0.5">
            Audit teacher details, registry activation state, anti-cheat
            focuses, response sheets, and duration logs.
          </p>
        </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody>
                  {session.participants.map((p: any, idx: number) => {
                    const scorePercentage =
                      totalQuestions > 0 && p.score !== null
                        ? (p.score / totalQuestions) * 100
                        : 0;
                    ...
                    const answersArray = Array.isArray(assessmentQuestions)
                      ? assessmentQuestions.map((q: any, qIdx: number) => {
                          const studentAnswer =
                            studentAnswers[q.id] ||
                            studentAnswers[String(qIdx + 1)] ||
                            studentAnswers[String(qIdx)] ||
                            studentAnswers[`q-${qIdx + 1}`] ||
                            studentAnswers[`q_${qIdx + 1}`] ||
                            "No Answer";
                        ...
                        <tr className="hover:bg-gray-50/30 transition-all border-b border-gray-100">
                          ...
                          <td className="px-6 py-4 text-center">
                            {p.terminated ? (
                              ...
                              <span className="inline-flex h-6 w-6 rounded-none bg-amber-100 border border-amber-300 text-amber-600 font-black items-center justify-center text-[10px]">
                                1st
                              </span>
                            ) : idx === 1 && p.score !== null ? (
                              <span className="inline-flex h-6 w-6 rounded-none bg-gray-100 border border-gray-300 text-gray-600 font-black items-center justify-center text-[10px]">
                                2nd
                              </span>
                            ) : idx === 2 && p.score !== null ? (
                              <span className="inline-flex h-6 w-6 rounded-none bg-orange-100 border border-orange-300 text-orange-700 font-black items-center justify-center text-[10px]">
                                3rd
                              </span>
                            ) : (
                          ...
                          <td className="px-6 py-4">
                            {p.terminated ? (
                              <span className="font-extrabold text-red-500 text-xs uppercase">
                                Disqualified
                              </span>
                            ) : p.score !== null ? (
                              ...
                              <div className="space-y-4 animate-in slide-in-from-top-2 duration-200">
                                ...
                                {answersArray.length === 0 ? (
                                  <p className="text-xs text-gray-400 italic">
                                    No submissions reported for this educator.
                                  </p>
                                ) : (
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {answersArray.map(
                                      (ans: any, aIdx: number) => (
                                        ...
                                        >
                                          <span
                                            className={`h-5 w-5 rounded-none flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5 border ${
                                              ans.isCorrect
