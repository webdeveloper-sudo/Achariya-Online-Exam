...
export default function DirectorTeacherDetailsPage({ params }: { params: Promise<{ email: string }> }) {
  ...
  const fetchTeacherTimeline = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/director/teachers/${encodedEmail}`, {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.message || "Failed to load educator timeline");
      }
    } catch (err) {
  ...
  if (error || !profile) {
    return (
      <div className="p-8 space-y-6 max-w-2xl mx-auto text-center">
        ...
        <p className="text-gray-550 text-sm">{error || "The requested educator profile is unavailable."}</p>
        <Link
        ...
        >
          <ArrowLeft size={16} /> Return to Educator Directory
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom duration-300">
      ...
      <div className="flex items-center gap-4">
        <Link
          href="/director/teachers"
          className="p-3 bg-white border border-gray-200 rounded-none hover:bg-gray-50 text-gray-500 hover:text-gray-900 transition-colors shadow-sm cursor-pointer"
        >
          <ArrowLeft size={16} />
        </Link>
        <div>
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Educator Directory</span>
          <h2 className="text-2xl font-black mt-1 text-gray-900">Teacher Profile</h2>
        </div>
      </div>
        ...
        <div className="flex-1 space-y-4">
          <div className="flex items-start justify-between flex-wrap gap-2">
            <div>
              ...
              <p className="text-xs text-blue-600 font-bold mt-1">Employee ID: {profile.userId}</p>
            </div>
            
            {/* Activation Status Badge */}
            <div>
              {profile.activated ? (
                <span className="inline-flex items-center px-3 py-1 rounded-none text-xs font-bold bg-emerald-50 text-emerald-600 border border-emerald-200">
                  Activated teacher
                </span>
              ) : (
                <span className="inline-flex items-center px-3 py-1 rounded-none text-xs font-bold bg-amber-50 text-amber-600 border border-amber-200 animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-2 border-t border-gray-200">
            ...
            <div className="flex items-center gap-2.5 text-xs text-gray-600">
              ...
              <span>Qualifications: {profile.qualification || "None reported"}</span>
            </div>
            <div className="flex items-center gap-2.5 text-xs text-gray-600">
              <Briefcase size={14} className="text-gray-400 shrink-0" />
              <span>Designation: {profile.designation || "N/A"}</span>
            </div>
            <div className="flex items-center gap-2.5 text-xs text-gray-600 col-span-2">
              <School size={14} className="text-gray-400 shrink-0" />
              <span>Branch Hub: {profile.branch || "Branch Hub N/A"}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-black text-gray-900">Chronological Evaluation Timeline</h3>
          <p className="text-xs text-gray-500 mt-1">
            Browse teacher&apos;s historical attempts, comparative accuracy scores, integrity indexes, and detailed answer audits.
          </p>
        </div>

        {attempts.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-none p-12 text-center text-gray-400 font-bold shadow-sm">
            No evaluation attempts found for this educator.
          </div>
        ) : (
          <div className="space-y-6">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-gray-500">
                        Focus: <span className="font-bold text-gray-700">{attempt.assessment.position}</span> · Target: <span className="font-bold text-gray-700">{attempt.assessment.recruitmentFor}</span> · Dept: <span className="font-bold text-gray-700">{attempt.assessment.department} ({attempt.assessment.teaching})</span>
                      </p>
                    </div>
                            ...
                            <div key={aIdx} className={`p-4 border rounded-none flex items-start gap-3 ${
                              ...
                              <div className="text-xs space-y-1">
                                ...
                                <p className="text-[11px] text-gray-500 mt-1">
                                  Educator Selection: <span className="font-extrabold text-gray-700">{ans.studentAnswer || "No Answer"}</span>
                                </p>
                                {!ans.isCorrect && (
