...
import { Lock, Mail, Shield, AlertCircle, HelpCircle, Eye, EyeOff } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          res.status === 500
            ? "Server error (500). Please check database connectivity or restart the development server."
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white border border-gray-300 rounded-none pl-12 pr-11 py-3 text-sm focus:border-blue-600 outline-none text-gray-900 placeholder-gray-400 font-semibold"
                  placeholder="••••••••"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 transition-colors cursor-pointer focus:outline-none p-1"
