...
export default function CandidateLivePage() {
  ...
  // Restore candidate identity and cached answers from localStorage
  useEffect(() => {
    if (!token) return;
    const localDataStr = localStorage.getItem(`recruitment_candidate_${token}`);
    if (localDataStr) {
      try {
        const data = JSON.parse(localDataStr);
        if (data.participantId && data.candidateInfo) {
  ...
  const pollSession = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch(`/api/recruitment/live/${token}/status`);
      if (!res.ok) return;
  ...
  const registerTabSwitch = async () => {
    ...
    // Notify backend
    try {
      const res = await fetch(`/api/recruitment/live/${token}/tabevent`, {
        method: "POST",
  ...
  const saveToLocalState = (
    ...
    if (!token || !participantId || !candidateInfo) return;
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [sessionStatus, participantId, candidateInfo, isTerminated, submitted]);
  ...
  const handleLookupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/recruitment/live/${token}/lookup?query=${encodeURIComponent(lookupQuery.trim())}`);
      const data = await res.json();
      if (!res.ok || data.success === false) {
        throw new Error(data.message || "Candidate profile not found.");
      }

      setFormName(data.candidate.name || "");
      setFormEmail(data.candidate.email || "");
      setFormPhone(data.candidate.phone || "");
      setFormQualification(data.candidate.qualification || "");
      setFormExperience(data.candidate.experience || "");

      setIsPrefilled(true);
      setShowLookupModal(false);
      setJoinError(null);
    } catch (err: any) {
      setLookupError(err.message || "No matching candidate profile found.");
    } finally {
  ...
  const handleJoinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/recruitment/live/${token}/join`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: formName,
      ...
      // Duplicate candidate detected — auto-open the lookup/retrieve modal
      if (res.status === 409 && data.duplicate) {
        setJoinError(null);
        setLookupQuery(formEmail || formPhone);
        setLookupError(null);
      ...
      if (!res.ok) {
        throw new Error(data.message || "Failed to join recruitment waitlist.");
      }
      ...
      const cInfo = {
        ...
        experience: formExperience || "None",
      };
      ...
      setJoinError(err.message || "Could not register applicant profile.");
    } finally {
  ...
  const handleManualSubmit = async () => {
    if (!confirm("Are you sure you want to finalize and submit your responses? This action is irreversible.")) return;
  ...
  const submitAssessment = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/recruitment/live/${token}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          participantId,
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // TERMINATED SCREEN (DISQUALIFIED CANDIDATE - Tab switches exceed limit)
  // ───────────────────────────────────────────────────────────────────────────
  if (isTerminated) {
    return (
        ...
        <div className="max-w-md w-full bg-white border border-red-200 rounded-none p-8 text-center space-y-6 shadow-sm">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">
              We noticed multiple tab switches or focus switches. To preserve exam integrity, this hiring evaluation session has been locked.
            </p>
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-none text-left space-y-2 mt-4">
              <p className="text-xs text-gray-500">Applicant: <span className="font-bold text-gray-700">{candidateInfo?.name}</span></p>
              <p className="text-xs text-gray-500">Position: <span className="font-bold text-blue-600">{assessment?.position}</span></p>
            ...
            <p className="text-sm text-emerald-600 font-medium mt-4">
              Thank you! Our recruiters will review your files and reach out to you soon.
            </p>
          </div>

          <button
            onClick={() => {
              localStorage.removeItem(`recruitment_candidate_${token}`);
              router.push("/");
            }}
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // THANK YOU SCREEN (SUCCESSFULLY SUBMITTED - strictly hides score & rank)
  // ───────────────────────────────────────────────────────────────────────────
  if (submitted) {
    return (
      <div className="min-h-screen bg-gray-50 font-sans flex relative overflow-hidden items-center justify-center p-6">
        <div className="max-w-md w-full bg-white border border-gray-200 rounded-none p-8 text-center space-y-6 shadow-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-none text-left space-y-2 mt-4">
              <p className="text-xs text-gray-500">Applicant: <span className="font-bold text-gray-700">{candidateInfo?.name}</span></p>
              <p className="text-xs text-gray-500">Applied Position: <span className="font-bold text-blue-600">{assessment?.position}</span></p>
              <p className="text-xs text-gray-500">Institution: <span className="font-bold text-gray-700">{assessment?.recruitmentFor}</span></p>
            </div>
            <p className="text-sm text-emerald-600 font-bold mt-6 leading-relaxed">
              Thank you! Our recruiters will reach out to you soon.
            </p>
          </div>
      ...
      return (
        <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center text-gray-900 p-6">
          <div className="max-w-md w-full bg-white border border-gray-200 rounded-none p-8 text-center space-y-4 shadow-sm">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">
              Entry is currently locked. The recruiting host has already initiated this assessment session.
            </p>
      ...
      return (
        <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center text-gray-900 p-6">
          <div className="max-w-md w-full bg-white border border-gray-200 rounded-none p-8 text-center space-y-4 shadow-sm">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">
              This conducted recruitment session is complete and has been finalized by the host.
            </p>
            <button onClick={() => router.push("/")} className="w-full bg-white hover:bg-gray-50 border border-gray-300 py-3 rounded-none font-bold text-sm text-gray-700 transition-all shadow-sm">
              Return Home
            </button>
      ...
      <div className="min-h-screen bg-gray-50 font-sans flex relative overflow-hidden items-center justify-center p-6">
        <div className="max-w-md w-full bg-white border border-gray-200 rounded-none p-8 space-y-6 shadow-sm relative z-10">
          <div className="text-center space-y-2">
            ...
            <h2 className="text-2xl font-black tracking-tight text-gray-900">Recruitment Portal</h2>
            <p className="text-xs text-gray-500">Join the live applicant evaluation for <span className="font-bold text-emerald-600">{assessment?.recruitmentFor}</span></p>
          </div>

          <div className="bg-gray-50 border border-gray-200 rounded-none p-4 flex items-center justify-between">
            <div>
              <p className="text-[10px] text-gray-400 uppercase tracking-wider font-bold">Role Evaluation</p>
              <h3 className="text-sm font-bold text-gray-700 truncate mt-0.5 max-w-[200px]">{assessment?.position}</h3>
            </div>
          ...
          <form onSubmit={handleJoinSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white border border-gray-300 rounded-none pl-11 pr-4 py-3 text-sm focus:border-blue-600 outline-none transition-all placeholder:text-gray-400 font-bold text-gray-900"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-wider text-gray-500">Qualification</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-gray-300 rounded-none pl-9 pr-3 py-3 text-sm focus:border-blue-600 outline-none transition-all placeholder:text-gray-400 font-bold text-xs text-gray-900"
                  />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-wider text-gray-500">Experience (Yrs)</label>
                <div className="relative">
            ...
            <div className="pt-2 text-center">
              ...
              >
                Already onboarded? Enter your email/mobile
              </button>
            </div>
          </form>
        </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[250] flex items-center justify-center p-4">
            <div className="max-w-md w-full bg-white border border-gray-200 rounded-none p-6 space-y-6 shadow-sm relative animate-in fade-in zoom-in-95 duration-200">
              <div className="text-center space-y-2">
                ...
                <p className="text-xs text-gray-500">Enter your registered email or 10-digit mobile number to pull up your profile details.</p>
              </div>
              ...
              {isDuplicateFlow && (
                <div className="p-3 bg-amber-50 border border-amber-300 text-amber-700 rounded-none text-xs flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5">A candidate profile with this email or mobile already exists. Enter your email or mobile below to retrieve your account and join the waiting room.</p>
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleLookupSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-wider text-gray-500">Email or Mobile Number</label>
                  <input
                    required
                    type="text"
      ...
      <div className="min-h-screen bg-gray-50 font-sans flex relative overflow-hidden items-center justify-center p-6">
          ...
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[200] flex items-center justify-center p-6">
            <div className="max-w-sm w-full bg-white border border-amber-300 rounded-none p-8 text-center space-y-6 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-500 leading-relaxed">
                  This hiring assessment must be completed under fullscreen security. Return to continue waiting.
                </p>
              </div>
              <button
        ...
        <div className="max-w-md w-full bg-white border border-gray-200 rounded-none p-8 text-center space-y-6 shadow-sm relative z-10">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">
              Hello <span className="font-extrabold text-gray-900">{candidateInfo?.name}</span>. You have joined the queue successfully. The recruiting panel will begin the exam shortly.
            </p>
          </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-none p-4 divide-y divide-gray-100 text-left text-xs space-y-2">
            <div className="py-1.5 flex justify-between">
              ...
              <span className="font-bold text-gray-700">{candidateInfo?.name}</span>
            </div>
            <div className="py-1.5 flex justify-between">
              <span className="text-gray-400">Applied Role</span>
              <span className="font-bold text-gray-700">{assessment?.position}</span>
            </div>
            <div className="py-1.5 flex justify-between">
              <span className="text-gray-400">Email</span>
              <span className="font-bold text-gray-700">{candidateInfo?.email}</span>
            </div>
            <div className="py-1.5 flex justify-between">
              <span className="text-gray-400">Qualification</span>
              <span className="font-bold text-gray-700">{candidateInfo?.qualification}</span>
            </div>
          </div>
        ...
        <header className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-gray-200 p-4 z-40 shadow-sm">
          <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-[10px] text-gray-400 mt-0.5">{candidateInfo?.name} (Candidate Profile)</p>
            </div>
        ...
        <main className="flex-1 max-w-3xl w-full mx-auto p-6 space-y-6">
          ...
          <div className="space-y-6 pb-20">
            {questions.map((q: any, idx: number) => {
              const qKey = q.id !== undefined && q.id !== null ? String(q.id) : `q_${idx + 1}`;
              const hasOptions =
              ...
              return (
                <div key={qKey} className="bg-white border border-gray-200 rounded-none p-6 md:p-8 space-y-6 shadow-sm">
                  ...
                  {hasOptions ? (
                      ...
                      {(q.options && q.options.length > 0 ? q.options : ["True", "False"]).map((opt: string, optIdx: number) => {
                        ...
                        return (
                          <button
                            key={optIdx}
                            onClick={() => {
                              const newAnswers = { ...answers, [qKey]: opt };
                    ...
                    <div className="w-full">
                      <input
                        ...
                        value={answers[qKey] || ""}
                        onChange={(e) => {
                          const newAnswers = { ...answers, [qKey]: e.target.value };
                          setAnswers(newAnswers);
                          saveToLocalState(newAnswers, tabSwitches, isTerminated, submitted);
                        }}
