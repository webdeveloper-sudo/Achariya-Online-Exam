...
export async function GET(
  ...
  try {
    ...
    // Auth Guard: Only Admin OR the Teacher who conducted the session can view details
    const isAdmin = decoded.role === "Admin";
    const isHost = decoded.id === session.teacherId;

    if (!isAdmin && !isHost) {
      return NextResponse.json({ message: "Access Denied: You do not have permissions to view this session." }, { status: 403 });
    }

    // Fetch Host Teacher profile
