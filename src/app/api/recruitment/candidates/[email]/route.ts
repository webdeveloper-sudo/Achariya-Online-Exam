...
export async function GET(
  ...
  try {
    ...
    const attempts = await prisma.recruitmentLiveSessionParticipant.findMany({
    ...
    if (attempts.length === 0) {
      return NextResponse.json({
        success: false,
        message: "No recruitment records found for this candidate.",
      }, { status: 404 });
    }

    // Retrieve the most recent registration to establish candidate profile
    ...
    const candidateProfile = {
      ...
      experience: latestAttempt.experience,
    ...
    console.error("Error fetching detailed candidate timeline:", error);
    return NextResponse.json(
