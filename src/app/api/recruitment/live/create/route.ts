...
export async function POST(request: Request) {
  try {
    ...
    const assessment = await prisma.recruitmentAssessment.findUnique({ where: { id: assessmentId } });
    if (!assessment) {
      return NextResponse.json({ message: "Recruitment assessment not found." }, { status: 404 });
    }
    ...
    const session = await prisma.recruitmentLiveSession.create({
      data: {
        token,
        assessmentId,
    ...
    const joinUrl = `${APP_URL}/live/recruitment/${token}`;
    const hostUrl = `${APP_URL}/live/recruitment/${token}/host`;
