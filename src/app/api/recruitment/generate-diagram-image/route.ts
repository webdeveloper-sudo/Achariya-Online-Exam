...
export async function POST(request: Request) {
  try {
    ...
    const imagePrompt = `
Create a clear, educational diagram or illustration relevant to this job candidate evaluation exam question:
...
- Suitable for a candidate evaluation exam paper
- No text that contradicts the question
`.trim();

    for (const modelName of imageModels) {
      try {
        console.log(`[Recruiter DiagramGen] Trying model: ${modelName}`);
        ...
        for (const candidate of candidates) {
          ...
          for (const part of parts) {
            if (part.inlineData?.mimeType?.startsWith("image/")) {
              ...
              console.log(`[Recruiter DiagramGen] Successfully generated image with model: ${modelName}`);
              return NextResponse.json({ success: true, base64: base64DataUrl });
            }
          }
        }
      } catch (err: any) {
        console.warn(`[Recruiter DiagramGen] Model ${modelName} failed:`, err.message || err);
      }
    }
    ...
    console.log("[Recruiter DiagramGen] Image models failed. Attempting SVG fallback...");
    ...
    console.error("[Recruiter DiagramGen] Unexpected error:", error);
...
3. The SVG must:
   ...
   - Look modern, colored, and visually appealing for a candidate assessment paper.
`.trim();
  ...
  for (const modelName of textModels) {
    try {
      console.log(`[Recruiter DiagramGen SVG Fallback] Trying model: ${modelName}`);
      const response = await ai.models.generateContent({
        model: modelName,
        contents: prompt,
      });
      ...
      if (startIndex !== -1 && endIndex !== -1) {
        ...
        console.log(`[Recruiter DiagramGen SVG Fallback] Successfully generated SVG using ${modelName}`);
        return base64DataUrl;
      }
    } catch (err: any) {
      console.warn(`[Recruiter DiagramGen SVG Fallback] Model ${modelName} failed:`, err.message || err);
    }
  }
  return null;
}
