...
export async function POST(request: Request) {
  try {
    ...
    const { questionText, context, recruitmentFor } = body;
    ...
    const prompt = `
Create a clean, professional, and educational SVG diagram representing the visual context for this recruitment assessment question:
"${questionText}"

${context ? `Additional Context/Guidelines: ${context}` : ""}
${recruitmentFor ? `Target Role/Domain: ${recruitmentFor}` : ""}
...
3. The SVG must:
   ...
   - Look professional, modern, and high-contrast for a hiring assessment paper.
`.trim();
