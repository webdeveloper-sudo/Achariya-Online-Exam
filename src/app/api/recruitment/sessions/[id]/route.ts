...
export async function GET(
  ...
  try {
    ...
    // Fetch the recruitment live session
    ...
    // Auth Guard: Only Admin OR the Recruiter who conducted the session can view details
    const isAdmin = decoded.role === "Admin";
    const isHost = decoded.id === session.recruiterId;

    if (!isAdmin && !isHost) {
      return NextResponse.json({ message: "Access Denied: You do not have permissions to view this session." }, { status: 403 });
    }

    // Fetch Host Recruiter profile
    const hostRecruiter = await prisma.recruiter.findUnique({
