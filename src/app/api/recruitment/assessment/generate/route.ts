...
import { parseQuestionPaperPdf } from "@/lib/pdf-question-parser";
...
export async function POST(request: Request) {
  try {
    ...
    const pdfClassification = (
      formData.get("pdfClassification")?.toString() ||
      formData.get("pdfType")?.toString() ||
      ""
    ).trim();
    const contextText = (formData.get("contextText")?.toString() || "").trim();

    // ── Direct Question Paper Parsing Pipeline (Zero AI) ──────────────────
    if (
      ...
      if (validFiles.length === 0) {
        return NextResponse.json(
          {
            success: false,
            message: "Please upload a Question Paper PDF document.",
          },
          { status: 400 },
        );
      }
      const primaryFile = validFiles[0];
      const fileExt = primaryFile.name.split(".").pop()?.toLowerCase();
      if (fileExt !== "pdf") {
        return NextResponse.json(
          {
            success: false,
            message: "Direct Question Paper parsing requires a PDF document.",
          },
          { status: 400 },
        );
      }
      ...
      const parseResult = await parseQuestionPaperPdf(pdfBuffer, {
        mode: pdfClassification,
        contextText,
      });
    ...
    const teachingType = (
      formData.get("teachingType")?.toString() || ""
    ).trim(); // teaching or non teaching
    let generationMode = (
      formData.get("generationMode")?.toString() || ""
    ).trim();
    ...
    if (
      (generationMode === "pdf_only" || generationMode === "pdf_context") &&
      validFiles.length === 0
    ) {
      return NextResponse.json(
        {
    ...
    const numQuestions = numQuestionsVal
      ? Math.min(
          Math.max(parseInt(numQuestionsVal.toString(), 10) || 10, 1),
          100,
        )
      : 10;
    ...
    if (generationMode === "pdf_only" || generationMode === "pdf_context") {
      ...
      if (extractedText.length > 15000 && contextText) {
        ...
        const keywords = cleanContextText
          .toLowerCase()
          .replace(/[^\w\s]/g, "")
          .split(/\s+/)
          .filter((w) => w.length > 3);
    ...
    let prompt = `
You are an expert recruitment manager and HR interviewer for the ACHARIYA Educational Group. Your goal is to design an assessment to evaluate job candidates.

TARGET CONFIGURATION:
- Role/Position Applied: ${position || "Candidate"}
- Department/Field: ${department || "General"}
- Job Classification: ${teachingType || "Teaching"}
- Core Topic/Skillset Focus: ${topic || "Core Aptitude & Competency"}
- Difficulty Level: ${difficulty || "Mixed"}

Pedagogical Context:
Make sure questions are directly relevant to evaluating the competency of a candidate applying for a "${position}" position in the "${department}" department.
${teachingType === "teaching" ? "Focus on pedagogical knowledge, teaching methodologies, classroom management, and subject proficiency." : "Focus on administration skills, task organization, work efficiency, logical aptitude, and role competency."}
...
`;
    ...
    for (const modelName of modelsToTry) {
      try {
        console.log(`[HR AI Gen] Trying model: ${modelName}`);
        ...
        console.warn(`[HR AI Gen] Model ${modelName} failed:`, err.message);
        lastError = err;
      }
    }
    ...
    // ── Resilient JSON Parsing & Auto-Repair ─────────────────────────────
    ...
    try {
      ...
      // 2. Auto-repair truncated JSON output
      try {
        const lastBrace = clean.lastIndexOf("}");
        if (lastBrace !== -1) {
          let candidate = clean.slice(0, lastBrace + 1);
          if (!candidate.includes('"questions"')) {
            candidate = `{"questions": [${candidate}]}`;
          } else if (!candidate.trim().endsWith("]}")) {
            candidate = candidate + "\n]}";
          }
        ...
        // 3. Fallback: Regex extraction of individual valid question objects
        const objectRegex =
          /\{[\s\r\n]*"id"[\s\S]*?"(?:explanation|correctAnswer)"\s*:\s*"(?:[^"\\]|\\.)*"\s*\}/g;
        let match;
    ...
    if (!questions || questions.length === 0) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Failed to parse AI response into strict JSON format.",
        },
        { status: 500 },
      );
    }
    ...
    console.error("Error generating recruitment questions:", error);
    return NextResponse.json(
