...
export async function POST(request: Request) {
  try {
    ...
    // ── 2. Direct Question Paper Parsing Pipeline (Zero AI) ────────────────
    if (
    ...
    // ── 0. Guard: API key for AI generation ────────────────────────────────
    ...
    const assessmentStyle = (
      formData.get("assessmentStyle")?.toString() || ""
    ).trim();
    let generationMode = (
        ...
        return NextResponse.json(
          {
            success: false,
            message:
              "Invalid request. Please upload a document, enter instructions, or choose a mode.",
          },
          { status: 400 },
        );
      }
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Contextual instructions are required for Text Only generation mode.",
        },
        { status: 400 },
      );
    }
        ...
        return NextResponse.json(
          {
            success: false,
            message:
              "Could not extract text from the document. It might be empty or unreadable.",
          },
          { status: 400 },
        );
      }
...
`;
    ...
    // Active high-availability Gemini models with separate independent quotas
    const modelsToTry = [
      "gemini-2.5-flash",
      "gemini-2.0-flash",
    ...
    for (const modelName of modelsToTry) {
      try {
        console.log(
          `Attempting question generation with Gemini model: ${modelName}`,
        );
        ...
        if (text && text.trim().length > 0) {
          textResponse = text;
          console.log(
            `Successfully generated questions using model: ${modelName}`,
          );
          break;
        }
      } catch (err: any) {
        console.warn(
          `Gemini Model ${modelName} failed or hit quota limits:`,
          err.message || err,
        );
        lastError = err;
      }
    ...
    if (!textResponse) {
      throw (
        lastError ||
        new Error("All fallback Gemini models failed to generate content.")
      );
    }
