...
`.trim();
    ...
    for (const modelName of imageModels) {
      try {
        console.log(`[Teacher DiagramGen] Trying model: ${modelName}`);
        ...
        for (const candidate of candidates) {
          ...
          for (const part of parts) {
            if (part.inlineData?.mimeType?.startsWith("image/")) {
              ...
              console.log(`[Teacher DiagramGen] Successfully generated image with model: ${modelName}`);
    ...
    console.log("[Teacher DiagramGen] Image models failed. Attempting SVG fallback...");
...
`.trim();
  ...
  for (const modelName of textModels) {
    try {
      console.log(`[Teacher DiagramGen SVG Fallback] Trying model: ${modelName}`);
      const response = await ai.models.generateContent({
        model: modelName,
        contents: prompt,
      });
      ...
      if (startIndex !== -1 && endIndex !== -1) {
        ...
        console.log(`[Teacher DiagramGen SVG Fallback] Successfully generated SVG using ${modelName}`);
        return base64DataUrl;
      }
    } catch (err: any) {
      console.warn(`[Teacher DiagramGen SVG Fallback] Model ${modelName} failed:`, err.message || err);
    }
  }
  return null;
}
