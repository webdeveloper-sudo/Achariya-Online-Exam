  ...
  try {
    ...
    if (session.participants.length === 0) {
      return NextResponse.json(
        { message: "At least one teacher must be in the waiting room." },
        { status: 400 }
      );
    }

    const updated = await prisma.directorLiveSession.update({
      where: { token },
      data: {
        status: "ACTIVE",
        startedAt: new Date(),
    ...
    return NextResponse.json({
      success: true,
      message: "Director assessment session initiated.",
      startedAt: updated.startedAt,
    });
  } catch (error: any) {
    console.error("Error starting live director session:", error);
    return NextResponse.json(
