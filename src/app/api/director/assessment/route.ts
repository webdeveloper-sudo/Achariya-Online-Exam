...
export async function GET(request: Request) {
  try {
    ...
    // Retrieve assessments created by this director
    const myAssessments = await prisma.directorAssessment.findMany({
    ...
    return NextResponse.json({
      success: true,
      myAssessments,
      publicAssessments: [] // Director assessments are not pooled/shared
    });
  } catch (error: any) {
    console.error("Error retrieving director assessments:", error);
    return NextResponse.json(
