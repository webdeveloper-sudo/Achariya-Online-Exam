...
export default function RecruiterSessionsPage() {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      console.error("Error loading recruiter sessions:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalCandidates = sessions.reduce((acc, curr) => acc + curr.stats.participantCount, 0);
  const avgTurnout = totalSessions > 0 ? (totalCandidates / totalSessions).toFixed(1) : "0";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Browse and audit all real-time candidate examination rooms initiated under your account.
          </p>
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="p-2.5 bg-white border border-gray-300 rounded-none text-gray-500 hover:text-gray-900 hover:bg-gray-50 transition-colors shadow-sm"
          >
      ...
      {loading ? (
        ...
        <div className="bg-white border border-dashed border-gray-300 rounded-none h-80 flex flex-col items-center justify-center gap-3 text-gray-400">
          ...
          <p className="text-xs text-gray-400 max-w-sm text-center leading-relaxed">
            Invite candidates in real-time by going to the <Link href="/recruitment/assessments" className="text-blue-600 font-bold hover:underline">Assessments Repository</Link> and clicking &quot;Host Live&quot; on any paper!
          </p>
        </div>
      ) : (
            ...
            return (
              <div key={session.id} className="bg-white/80 border border-gray-200 rounded-none p-6 hover:border-blue-300 hover:shadow-md transition-all shadow-sm backdrop-blur-sm flex flex-col justify-between min-h-[240px]">
                ...
                <div className="flex gap-2">
                  <Link
                    href={`/recruitment/sessions/${session.id}`}
