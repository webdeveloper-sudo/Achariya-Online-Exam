...
import {
  ...
  ChevronDown, ChevronUp, AlertCircle, HelpCircle, Check, X, Shield, Mail, Phone, Award
} from "lucide-react";
...
export default function RecruiterLiveSessionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !session) {
    return (
      <div className="p-8 space-y-6 max-w-2xl mx-auto text-center">
        ...
        <p className="text-gray-500 text-sm">{error || "The session report is unavailable or access was denied."}</p>
        <Link
          href="/recruitment/sessions"
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom duration-300">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Role: <span className="font-bold text-gray-700">{session.assessment.position}</span> · Institution: <span className="font-bold text-gray-700">{session.assessment.recruitmentFor}</span> · Room key: <span className="font-mono text-blue-600 font-bold">{session.token}</span>
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div className="bg-white/80 border border-gray-200 rounded-none p-5 shadow-sm backdrop-blur-sm">
          <div className="flex items-center gap-3 text-blue-600 mb-3">
            <Users size={16} />
            <span className="text-[9px] font-bold uppercase tracking-wider text-gray-400">Total Applicants</span>
          </div>
      ...
      <div className="space-y-4">
        <div>
          <h3 className="text-lg font-black text-gray-900">Candidate Leaderboard &amp; Response Audit</h3>
          <p className="text-xs text-gray-500 mt-0.5">
            Audit candidate parameters, anti-cheat focuses, response sheets, and duration logs.
          </p>
        </div>

        {session.participants.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-none p-12 text-center text-gray-400 shadow-sm">
            No candidates registered or joined this room.
          </div>
        ) : (
                      ...
                      <Fragment key={p.id}>
                        <tr className="hover:bg-gray-50/30 transition-all border-b border-gray-100">
                          ...
                          <td className="px-6 py-4">
                            <div>
                              <Link
                                href={`/recruitment/candidates/${encodeURIComponent(p.email)}`}
                          ...
                          <td className="px-6 py-4">
                            <div>
                              <p className="font-bold text-gray-700">{p.qualification || "N/A"}</p>
                              <p className="text-[10px] text-gray-400 flex items-center gap-1 mt-0.5">
                                <Award size={11} className="text-blue-500 shrink-0" />
                                <span>{p.experience || "No Experience reported"}</span>
                              </p>
                            </div>
                          </td>
                            ...
                            <td colSpan={7} className="px-8 py-6 border-t border-b border-gray-100">
                              <div className="space-y-4 animate-in slide-in-from-top-2 duration-200">
                                ...
                                {answersArray.length === 0 ? (
                                  <p className="text-xs text-gray-400 italic">No submissions reported for this applicant.</p>
                                ) : (
                                      ...
                                      <div key={aIdx} className={`p-4 border rounded-none flex items-start gap-3 ${
                                        ...
                                        <div className="text-xs space-y-1">
                                          ...
                                          <p className="text-[11px] text-gray-500 mt-1">
                                            Candidate Selection: <span className="font-extrabold text-gray-700">{ans.studentAnswer || "N/A"}</span>
                                          </p>
                                          {!ans.isCorrect && (
