...
export default function RecruiterGeneratePage() {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        markedQuestions.forEach((q, idx) => {
          ...
          fetch("/api/recruitment/generate-diagram-image", {
            method: "POST",
        ...
        setGeneratorError(
          data.message || "Failed to generate recruitment assessment.",
        );
        setGeneratorLoading(false);
      }
    } catch (err: any) {
  ...
  const openSaveModal = () => {
    ...
    if (firstUnconfiguredIdx !== -1) {
      ...
      setTimeout(() => {
        const el = document.getElementById(`recruitment-question-card-${firstUnconfiguredIdx}`);
        if (el) {
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-300">
      <div className="no-print">
        <h2 className="text-3xl font-black text-gray-900">
          AI Recruitment Generator
        </h2>
        <p className="text-sm text-gray-500 mt-1">
          Tailor recruitment exams by specifying the role, school, and
          department. Feed standard reference docs to Gemini AI to compile exact
          questions.
        </p>
      </div>
                    ...
                    {[
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <input
                            type="radio"
                            name="recruitmentPdfClassification"
                            value={opt.id}
        ...
        <div className="lg:col-span-3 space-y-6 relative h-full overflow-y-auto custom-scrollbar bg-white/20 border border-gray-200 rounded-none px-6 pb-6 space-y-4 shadow-sm backdrop-blur-sm">
          ...
          {generatorLoading ? (
            <div className="bg-white border border-gray-200 rounded-none p-16 text-center text-gray-500 h-1/2 flex flex-col items-center justify-start space-y-6 no-print shadow-sm min-h-[350px]">
              ...
              <Loader
                variant="inline"
                message="Compiling candidate assessment..."
            ...
            <div className="bg-white border border-red-200 p-12 text-center space-y-5 no-print shadow-sm">
              ...
              <div className="space-y-2 max-w-lg mx-auto">
                ...
                <p className="text-[11px] text-gray-500">
                  Please adjust the question count, position criteria, or
            ...
            <div className="bg-white/40 border border-dashed border-gray-300 rounded-none p-16 text-center text-gray-500 space-y-2 no-print">
              ...
              <p className="text-xs max-w-sm mx-auto text-gray-500">
                Fill in the candidate targeting parameters, specify the hiring
                position, select a generation mode, and execute the generator.
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="space-y-4 no-print">
                ...
                {generatedQuestions.map((q, idx) => (
                  <div
                    id={`recruitment-question-card-${idx}`}
                    key={q.id}
                  ...
                  >
                    ...
                    <div className="text-xs font-bold uppercase tracking-wider text-blue-600 flex items-center gap-1">
                      ...
                      {q.requiresReview && (
                        <span className="text-[9px] font-black uppercase bg-amber-100 text-amber-800 border border-amber-300 px-1.5 py-0.5 rounded-none ml-2">
                          Needs Review
                        </span>
                      )}
                            ...
                            {q.options.map((opt, oIdx) => {
                              ...
                              return (
                                ...
                                >
                                  <input
                                    type="radio"
                                    name={`rec_q_${idx}_correctAnswer`}
                                    value={opt || letter}
                                    checked={isSelected}
                                    onChange={() => {
                                      const updated = [...generatedQuestions];
              ...
              <div className="print-only-layout hidden print:block bg-white text-black p-8 font-sans space-y-6">
                <div className="border-b-2 border-black pb-4 text-center">
                  ...
                  <div className="grid grid-cols-2 text-left text-xs gap-x-8 gap-y-1 max-w-xl mx-auto mt-4">
                    <p>
                      <strong>Position:</strong> {printConfig.position}
                    </p>
                    <p>
                      <strong>Department:</strong>{" "}
                      {printConfig.department || "General"}
                    </p>
                    <p>
                      <strong>Classification:</strong>{" "}
                      {printConfig.classification}
                    </p>
                    <p>
                      <strong>Supervised By:</strong> {printConfig.generatedBy}
                    </p>
                    <p>
                      <strong>Duration:</strong> {printConfig.duration}
                    </p>
                    <p>
                      <strong>Print Date:</strong> {printConfig.date} (
                      {printConfig.day})
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-6 pt-4">
                  {generatedQuestions.map((q, idx) => (
                    <div key={q.id} className="space-y-2">
                      ...
                      {q.type === "true_false" && (
                        <p className="pl-4 text-xs font-bold text-slate-600">
                          Option: True / False
                        </p>
                      )}
            ...
            <form onSubmit={handleSaveAssessment} className="space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer group pt-2">
                ...
                <span className="text-xs text-gray-600 group-hover:text-gray-900 transition-colors">
                  Save as public template (visible to other HR accounts)
                </span>
              </label>
